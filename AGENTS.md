...
2. **Upstream stays updatable.** The pin lives only in `clawos.lock.json`. Every plugin declares `openclaw.compat.pluginApi`
...
## The kernel bar
`packages/clawos-kernel` and `packages/clawos-shared` are the kernel: every line reviewed, fewer lines is better, every export
doc-commented, reuse an upstream mechanism before adding a parallel one, no `as unknown as` across a plugin/RPC boundary,
SDK calls only in `packages/clawos-kernel/src/upstream/`.
