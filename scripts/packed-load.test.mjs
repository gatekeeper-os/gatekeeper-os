...
test('prepublication registry serves exact bytes with integrity and rejects writes/unknown packages', async () => {
  const root = mkdtempSync(join(tmpdir(), 'clawos-packed-registry-test-'));
  const bytes = Buffer.from('unchanged packed artifact fixture');
  const archive = join(root, 'package.tgz');
  writeFileSync(archive, bytes);
  writeFileSync(join(root, 'package.json'), JSON.stringify({ name: '@clawkeepers/shared', version: '0.1.0-beta.1' }));
  const registry = await packedRegistry([{ root, archive, name: '@clawkeepers/shared' }]);
  try {
    const metadata = await (await fetch(registry.url + '/@clawkeepers%2fshared')).json();
    ...
    assert.equal((await fetch(registry.url + '/@clawkeepers/shared', { method: 'PUT', body: '{}' })).status, 405);
