...
set -euo pipefail
VM_NAME="${GKOS_VM_NAME:-gkos-test}"
DRIVER="${GKOS_VM_DRIVER:-multipass}"
VM_USER="${GKOS_VM_USER:-tester}"
VM_SRC="${GKOS_VM_SRC:-/home/$VM_USER/src}"
...
#
# GKOS_VM_STATE_DIR relocates the state directory deliberately. It does not weaken the ownership check — the
...
if [ -n "${GKOS_VM_STATE_DIR:-}" ]; then
  STATE_DIR="$(cd "$GKOS_VM_STATE_DIR" 2>/dev/null && pwd || true)"
  [ -n "$STATE_DIR" ] || { echo "[vm] GKOS_VM_STATE_DIR does not exist: $GKOS_VM_STATE_DIR" >&2; exit 1; }
  case "$STATE_DIR" in
    */scripts/vm/.state) ;;
    *) echo "[vm] GKOS_VM_STATE_DIR must be a scripts/vm/.state directory, got $STATE_DIR" >&2; exit 1;;
  esac
  ...
  if [ -z "$mine" ] || [ "$mine" != "$theirs" ]; then
    echo "[vm] GKOS_VM_STATE_DIR is not a worktree of this repository: $STATE_DIR" >&2; exit 1
  fi
...
mp_up() {
  ...
  multipass launch 24.04 --name "$VM_NAME" --cpus "${GKOS_VM_CPUS:-2}" --memory "${GKOS_VM_MEM:-4G}" --disk "${GKOS_VM_DISK:-20G}" \
...
mp_sync() {
  ...
  multipass transfer -r "$tmp/src" "$VM_NAME:/tmp/gkos-src"
  multipass exec "$VM_NAME" -- sudo bash -c "rm -rf '$VM_SRC' && mv /tmp/gkos-src '$VM_SRC' && chown -R $VM_USER:$VM_USER '$VM_SRC'"
  rm -rf "$tmp"
}
...
ssh_target() { echo "${GKOS_VM_HOST:?set GKOS_VM_HOST=user@host for the ssh driver}"; }
...
nyi() { vm_die "driver '$DRIVER' not implemented yet — implement in scripts/vm/common.sh or use GKOS_VM_DRIVER=multipass|ssh"; }
