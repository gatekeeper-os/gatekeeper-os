...
test('release bump preserves excluded plugin pins and is preview-only until applied', () => {
  const root = mkdtempSync(join(tmpdir(), 'gatekeeper-os-version-test-'));
  ...
  try {
    ...
    put('packages/kernel/package.json', { name: '@gatekeeper-os/kernel', version: '0.1.0', private: false, publishConfig: { access: 'public' } });
    put('packages/kernel/openclaw.plugin.json', { id: 'gkos-kernel', version: '0.1.0' });
    put('gkos.lock.json', { plugins: { 'gkos-kernel': '0.1.0', 'gkos-gatekeeper-github': '0.1.0' }, upstream: { version: '2026.9.2' } });
    ...
    const lock = JSON.parse(plan.get(join(root, 'gkos.lock.json'))!);
    assert.equal(lock.plugins['gkos-kernel'], '0.1.0-beta.1');
    assert.equal(lock.plugins['gkos-gatekeeper-github'], '0.1.0');
    ...
    put('packages/kernel/package.json', { name: '@gatekeeper-os/kernel', private: true });
