...
- **The agent's own workspace, `exec`, and non-gatekeeper tools** are governed by OpenClaw's normal tool policy and sandbox settings, which deny runtime/filesystem/automation groups and browser in the baseline (`config/config.d/00-baseline.json5`). Blueprint provisioning and effective policy passed scoped runtime/messaging checks in `20260912-155827-phase-9`; full driver integration remains unaccepted. The OS does not mediate them.
...
Observed on unmodified OpenClaw2026.9.2 and published2026.9.4 after approval hooks but before the execute callback. Kernel-owned failure redaction cannot intercept this upstream failure path. [Reproduction record](../plans/upstream-native-approval-logging.md).
