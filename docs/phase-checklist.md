...
- [x] `gkos.lock.json` pins `openclaw@2026.9.2`; CI installs that version and prints `openclaw --version` — evidence: https://github.com/gatekeeper-os/gatekeeper-os/actions/runs/34160492903 (success, `dbb663c`)
...
- [x] Three skills (`write-gatekeeper`, `gkos-operator`, `write-blueprint`) drafted under `.agents/skills/` — evidence: supplied scaffold `7534893`; verified tracked paths, not newly published or applied
...
- [x] Linux drop-in carries OS environment without editing the upstream unit; required modes hold — `gkos.conf`, `upstream-unit.service`, `permissions.txt`.
...
- [x] Current branch CI passed — https://github.com/gatekeeper-os/gatekeeper-os/actions/runs/34167820496 (`88bbe26`), also PR CI 34167822735. 92 unit tests pass; 12 kernel conformance TODOs are not passes.
- [x] macOS full acceptance — https://github.com/gatekeeper-os/gatekeeper-os/actions/runs/34167820614 (`88bbe26`), artifact `20260907-224656-phase-1`: **exit 0, 25/25**, Darwin, full mode; install 65 seconds. Two healthy concurrent cells, backup round-trip, no doctor errors or critical audit findings. Downloaded evidence verified locally under `vm-artifacts/github-macos-34167820614/`.
- [x] Phase 1 completion checkpoint `phase-1` — Ubuntu/macOS gates passed; [PR #1](https://github.com/gatekeeper-os/gatekeeper-os/pull/1).

## Phase 2 — Contracts and kit

- [x] `gkos-shared` exports every contract from plan §4.3 with doc comments; TypeBox schemas for wire types — `2218843`, `src/schemas.test.ts`: 39 shared tests and typecheck pass. Process-local callbacks are explicitly excluded from wire transport.
...
- [x] Phase 2 completion checkpoint `phase-2` — [PR #2](https://github.com/gatekeeper-os/gatekeeper-os/pull/2); host-only acceptance `20260907-232940`, branch CI 34170233623 and PR CI 34170298843 passed.

## Phase 3 — Kernel and gkos-gatekeeper-fs
...
- [x] `gkos grant revoke` → tool absent on the next turn — evidence: same combined run, `cli-grant-revoke`, `grant-revoked`, and `revoked-tool-absent` all pass.
- [x] Every kernel grant/use/revoke step above appears in `gkos audit tail` — evidence: same combined run, `successful-call-audited`, `observation-audited`, `unknown-policy-audited`, `cli-audit-tail`, and `revocation-audited` pass.
...
- Milestone tag: [`phase-3`](https://github.com/gatekeeper-os/gatekeeper-os/tree/phase-3), published only after required CI and merge.

## Phase 4 — gkos-gatekeeper-github
...
- [ ] Scenario: comment (simulated) then summarize thread including the pending comment; `gkos approvals apply all` → comment live on GitHub — evidence:
...
- [ ] `gkos update` to current `latest` succeeds or rolls back cleanly
