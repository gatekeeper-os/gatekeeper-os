...
export default definePluginEntry({id:'clawos-install-hook-monitor',register(api){
  ...
  for(const [stage,priority] of [['before',1000],['after',-1000]])api.on('before_install',(e)=>{
    appendFileSync('/home/tester/.openclaw-kernel-test/os/install-events.jsonl',JSON.stringify({stage,targetPlugin:e.targetType==='plugin',directory:e.sourcePathKind==='directory',pluginRequest:typeof e.request?.kind==='string'&&e.request.kind.startsWith('plugin-')})+'\n',{mode:0o600});
  },{priority});
}});
