...
import { updateEffects, rollback } from '../../packages/clawos-cli/src/commands/update.js';
...
import { kernelRpcForCell } from '../../packages/clawos-cli/src/util/kernel-rpc.js';
...
const path = '/home/tester/.clawos/updates/default/current.json';
if (mode === 'recover') {
    ...
    const j: UpdateJournal = { format: 1, id, cell: 'default', from: lock.upstream.version, target, kernelSchema: lock.kernelSchema, state: 'preparing', step: 1, completed: [], startedAt: new Date().toISOString(), previousBinary: lock.runtimeBinary ?? realpathSync(cmd('sh', ['-c', 'command -v openclaw']).trim()), candidateBinary: join('/home/tester/.clawos/runtimes/default', id, 'bin/openclaw'), previousDropIn: existsSync(join(cell.dropInDir, 'zz-clawos-runtime.conf')) ? readFileSync(join(cell.dropInDir, 'zz-clawos-runtime.conf'), 'utf8') : null };
    const effects = updateEffects(cell, lock, j, '/unused-full-conformance-runner');
    effects.conformance = async () => {
        const state = join('/home/tester/.clawos/updates/default', id, 'probe');
