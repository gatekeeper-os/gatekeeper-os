...
if(process.argv[2]==='allow'){
  ...
  cfg.plugins.entries['clawos-kernel'].config.install.allowSources=['upload:'+saved.uploadId];
}else{
  cfg.skills={install:{allowUploadedArchives:true}};
  cfg.plugins.allow=cfg.plugins.allow.filter(x=>x!=='clawos-kernel-monitor');cfg.plugins.allow.push('clawos-install-hook-monitor');
