...
if(process.env.CLAWOS_TEST_MODE!=='full'||process.env.HOME!=='/home/tester'||process.cwd()!=='/home/tester/src')throw new Error('full VM run required');
const started=Date.parse(process.env.CLAWOS_TEST_START??'');
...
if(channel.runId!==process.env.CLAWOS_TEST_START||Object.values(channel.checks??{}).some(value=>value!==true)||Object.keys(channel.checks??{}).length<71)throw new Error('channel acceptance incomplete');
...
if(kernel.runId!==process.env.CLAWOS_TEST_START||Object.values(kernel.checks??{}).some(v=>v!==true)||requiredKernel.some(id=>kernel.checks[id]!==true))throw new Error('kernel lifecycle acceptance incomplete');
process.stdout.write(JSON.stringify({ok:true,runId:process.env.CLAWOS_TEST_START,suites:Object.fromEntries(required.map(id=>[id,suites.get(id).passed])),channelChecks:Object.keys(channel.checks).length,kernelChecks:Object.keys(kernel.checks).length})+'\n');
