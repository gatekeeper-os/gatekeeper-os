...
const required=['plugin-loads','hooks-fire','tool-narrowing','gate-blocks','rpc-methods','cli-mounted','health','fs-gatekeeper','install-gate','install-hook'];
...
if(channel.runId!==process.env.CLAWOS_TEST_START||Object.values(channel.checks??{}).some(value=>value!==true)||Object.keys(channel.checks??{}).length<30)throw new Error('channel acceptance incomplete');
process.stdout.write(JSON.stringify({ok:true,runId:process.env.CLAWOS_TEST_START,suites:Object.fromEntries(required.map(id=>[id,suites.get(id).passed])),channelChecks:Object.keys(channel.checks).length})+'\n');
