...
if(process.env.HOME!=='/home/tester'||process.cwd()!=='/home/tester/src'||process.env.OPENCLAW_STATE_DIR!=='/home/tester/.openclaw')throw new Error('VM required');
const state=process.env.OPENCLAW_STATE_DIR, checks={};
const invoke=(cmd,args,input)=>spawnSync(cmd,args,{encoding:'utf8',input,timeout:120000,maxBuffer:1024*1024});
const assert=(name,value)=>{checks[name]=!!value;if(!value){process.stdout.write(JSON.stringify({runId:process.env.CLAWOS_TEST_START,checks,failure:name})+'\n');process.exit(1);}};
const fixture='/home/tester/reviewed-install-fixture';mkdirSync(fixture,{recursive:true});
writeFileSync(join(fixture,'package.json'),JSON.stringify({name:'clawos-install-fixture',version:'1.0.0',type:'module',openclaw:{extensions:['./index.js']}}));
writeFileSync(join(fixture,'openclaw.plugin.json'),JSON.stringify({id:'clawos-install-fixture',name:'Inert install fixture',configSchema:{type:'object',additionalProperties:false,properties:{}}}));
writeFileSync(join(fixture,'index.js'),'export default { id:"clawos-install-fixture",register(){} };\n');
const projected=JSON.parse(readFileSync(join(state,'os/config.d/15-runtime.json'),'utf8'));
assert('no-filesystem-roots',projected.plugins.entries['gatekeeper-fs'].config.roots.length===0);
...
const target=join(state,'extensions/clawos-install-fixture');
...
writeFileSync(local,JSON.stringify({plugins:{entries:{'clawos-kernel':{config:{install:{allowSources:[fixture]}}}}}}),{mode:0o600});
result=invoke('clawos',['config','apply','--json']);
...
const unavailableFixture='/home/tester/unavailable-policy-fixture',unavailableTarget=join(state,'extensions/clawos-unavailable-fixture');
mkdirSync(unavailableFixture,{recursive:true});
writeFileSync(join(unavailableFixture,'package.json'),JSON.stringify({name:'clawos-unavailable-fixture',version:'1.0.0',type:'module',openclaw:{extensions:['./index.js']}}));
writeFileSync(join(unavailableFixture,'openclaw.plugin.json'),JSON.stringify({id:'clawos-unavailable-fixture',configSchema:{type:'object',additionalProperties:false,properties:{}}}));
writeFileSync(join(unavailableFixture,'index.js'),'export default { id:"clawos-unavailable-fixture",register(){} };\n');
...
cfg.plugins.entries['clawos-kernel'].config.install.allowSources.push(unavailableFixture);
cfg.security.installPolicy.exec.command='/nonexistent/clawos-policy';writeFileSync(process.env.OPENCLAW_CONFIG_PATH,JSON.stringify(cfg),{mode:0o600});
...
result=invoke('clawos',['config','apply','--force','--json']);
...
process.stdout.write(JSON.stringify({runId:process.env.CLAWOS_TEST_START,checks,fullPhaseAcceptance:false})+'\n');
