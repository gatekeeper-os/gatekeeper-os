...
if (process.env.GKOS_KERNEL_VM !== '1' || process.env.OPENCLAW_STATE_DIR !== '/home/tester/.openclaw-kernel-test' || process.env.OPENCLAW_CONFIG_PATH !== '/home/tester/.openclaw-kernel-test/openclaw.json' || process.cwd() !== '/home/tester/src') throw new Error('VM required');
const state=process.env.OPENCLAW_STATE_DIR, file=process.env.OPENCLAW_CONFIG_PATH;
if (process.argv[2] === 'no-hooks') {
  const cfg=JSON.parse(readFileSync(file,'utf8'));
  cfg.plugins.entries['gkos-kernel'].hooks.allowConversationAccess=false;
  ...
  const {fsTools}=await import('../../packages/gkos-gatekeeper-fs/src/tools.ts');
  const {fsResources}=await import('../../packages/gkos-gatekeeper-fs/src/resources.ts');
  writeFileSync(join(state,'os/gatekeepers.json'),JSON.stringify({version:1,gatekeepers:[{pluginId:'gkos-gatekeeper-fs',vendor:'fs',apiVersion:1,root:resolve('packages/gkos-gatekeeper-fs'),tools:fsTools,resources:fsResources}]}),{mode:0o600});
  const config={gateway:{mode:'local',bind:'loopback',port:19100,auth:{mode:'token',token:randomBytes(32).toString('hex')}},update:{auto:{enabled:false}},
    ...
    plugins:{allow:['gkos-kernel','gkos-gatekeeper-fs','gkos-kernel-monitor'],load:{paths:['packages/gkos-kernel','packages/gkos-gatekeeper-fs','test/fixtures/kernel-monitor'].map(p=>resolve(p))},entries:{
      'gkos-kernel':{enabled:true,hooks:{allowConversationAccess:true},config:{operators:[],install:{allowSources:[]}}},
      'gkos-gatekeeper-fs':{enabled:true,config:{roots:['/home/tester/kernel-resource']}},
      'gkos-kernel-monitor':{enabled:true,hooks:{allowConversationAccess:true}},
    }}};
  writeFileSync(file,JSON.stringify(config),{mode:0o600});
  mkdirSync('/home/tester/.gkos',{recursive:true,mode:0o700});
  const registryPath='/home/tester/.gkos/cells.json';
