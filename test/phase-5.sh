...
if [ "${GKOS_TEST_MODE:-full}" != approvals-live ]; then
...
pnpm --filter @gatekeeper-os/kernel... --filter @gatekeeper-os/gatekeeper-fs... --filter @gatekeeper-os/conformance... --filter @gatekeeper-os/cli build > /home/tester/phase5-build.log 2>&1
