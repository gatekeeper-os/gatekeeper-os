...
if [ "${GKOS_TEST_MODE:-full}" != runtime-checkpoint ]; then
...
writeFileSync('/home/tester/.openclaw/os/config.d/90-local.json5',JSON.stringify({plugins:{entries:{'gkos-gatekeeper-fs':{config:{roots:['/home/tester/update-resource']}}}}}),{mode:0o600});
JS
gkos config apply > /home/tester/phase7-config.log 2>&1
gkos grant add --agent main file:///home/tester/update-resource/ --json > /home/tester/phase7-grant.json 2>/home/tester/phase7-grant.stderr
printf 'PASS real-filesystem-grant\n'
gkos update --check --json > "$EV/availability.json"
...
gkos kernel status --json | jq '{healthy,kernelSchema,activeRuns,activeEffects,maintenance}' > "$EV/final-status.json"
