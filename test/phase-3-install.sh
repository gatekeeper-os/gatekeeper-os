...
clawos install --yes --json > "$EV/install-again.json" 2>/home/tester/install-again.stderr
jq -e '.changed == false and (.steps[] | select(.step == "plugins") | .state == "ok")' "$EV/install-again.json" >/dev/null
printf 'PASS install-idempotent\n'
clawos kernel status --json > /home/tester/installed-kernel-status.json 2>/home/tester/kernel-status.stderr
jq -e '.healthy == true and (.gatekeepers[] | select(.vendor == "fs") | .healthy == true)' /home/tester/installed-kernel-status.json >/dev/null
clawos grant list --json > /home/tester/installed-grants.json 2>/home/tester/grants.stderr
