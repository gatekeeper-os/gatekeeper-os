...
export function selectCell(){
 if(process.env.CLAWOS_KERNEL_VM!=='1'||process.cwd()!=='/home/tester/npm-acceptance')throw new Error('vm-required');
 const rows=JSON.parse(command('clawos',['cell','list','--json']));
 const matches=rows.filter(r=>r.name===process.env.CLAWOS_CELL);
 ...
 for(const [key,value]of Object.entries({OPENCLAW_STATE_DIR:cell.stateDir,OPENCLAW_CONFIG_PATH:file,CLAWOS_CELL:cell.name})){
 ...
 const catalog=process.env.CLAWOS_GATEKEEPER_CATALOG||join(cell.stateDir,'os/gatekeepers.json');
