...
const phase=process.argv[2]??'normal', reportPath=process.env.GKOS_SCENARIO_REPORT;
...
try {
  ...
  if(phase==='normal'){
    ...
    for(const id of ['gkos-kernel','gkos-gatekeeper-fs']) check('plugin-loaded-'+id,plugins.ok&&Array.isArray(loaded)&&loaded.some(p=>p.id===id&&p.enabled===true&&p.status==='loaded'));
    ...
    check('no-grant-narrowing',empty.names.every(names=>!names.some(n=>n.startsWith('gk_'))&&names.includes('os_list_grants')&&names.includes('os_request_access')&&!names.some(n=>nativeDenied.includes(n))));
    ...
    check('introduced-owner-only',grant.audience==='owner-only');
    check('shared-audience-grant-refused',await denied(paired.client,'os.grants.introduce',{agentId:'main',url:'file:///home/tester/kernel-resource/',audience:'shared'}));
    ...
    check('valid-grant-narrowing',listed.names.every(names=>names.filter(n=>n.startsWith('gk_')).length===3&&['os_list_grants','os_request_access','gk_fs_dir_list','gk_fs_file_read','gk_fs_file_write'].every(n=>names.includes(n))));
