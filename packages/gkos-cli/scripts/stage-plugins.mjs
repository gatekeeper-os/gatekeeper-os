...
const destination = join(root, 'packages/gkos-cli/dist/templates/plugins');
const typeboxLicense = readFileSync(join(root, 'packages/gkos-gatekeeper-fs/node_modules/typebox/license'), 'utf8');

for (const [dir, id] of [['gkos-kernel', 'gkos-kernel'], ['gkos-gatekeeper-fs', 'gkos-gatekeeper-fs']]) {
  ...
  await build({
    ...
    splitting:false,
    sourcemap:false,
    noExternal:[/@gatekeeper-os\//, /^typebox/],
...
await build({
  entry:{
    tools:join(root, 'packages/gkos-gatekeeper-fs/src/tools.ts'),
    resources:join(root, 'packages/gkos-gatekeeper-fs/src/resources.ts'),
  },
  outDir:metadata,
  format:['esm'],
  splitting:false,
...
writeFileSync(join(destination, 'catalog.json'), JSON.stringify({
  version: 1,
  gatekeepers: [{pluginId: 'gkos-gatekeeper-fs', vendor: 'fs', apiVersion: 1, tools: fsTools, resources: fsResources}],
}));
...
await build({
  entry:{'install-policy':join(root, 'packages/gkos-cli/src/policy-entry.ts')},
  outDir:destination,
  format:['esm'],
  outExtension:()=>({js:'.mjs'}),
  target:'node22',
...
copyFileSync(join(root, 'packages/gkos-gatekeeper-fs/node_modules/typebox/license'), join(destination, 'THIRD-PARTY-NOTICES'));
