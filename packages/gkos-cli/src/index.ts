...
const COMMANDS: Record<string, Command> = {
  ...
  config: async (args, globals) => {
    ...
    console.error("usage: gkos config apply [--dry-run] [--force]");
    return 2;
  },
};
...
function usage(): number {
  console.error(
    [
      "usage: gkos <command> [--cell <name>] [--json] [--yes]",
      "",
...
export async function main(argv: string[]): Promise<void> {
  ...
  try {
    ...
    if (error instanceof StepError) {
      console.error(`gkos: ${error.message}`);
      if (error.hint) console.error(`  hint: ${error.hint}`);
    } else {
      console.error(`gkos: ${error instanceof Error ? error.message : String(error)}`);
    }
    code = 1;
  }
  process.exit(code);
