...
export function projectPlugins(cell: Cell, templates: string): { changed: boolean; versions: Record<string, string> } {
  ...
  for (const id of ["gkos-kernel", "gkos-gatekeeper-fs"]) versions[id] = JSON.parse(readFileSync(join(root, id, "package.json"), "utf8")).version;
  ...
  const fragment = {
    plugins: { allow: ["gkos-kernel", "gkos-gatekeeper-fs"], load: { paths: [join(root,"gkos-kernel"),join(root,"gkos-gatekeeper-fs")] }, entries: {
      "gkos-kernel": { enabled: true, hooks: { allowConversationAccess: true }, config: { operators: [], install: { allowSources: [], allowHashes: [] }, egress: { denyPatterns: ["grant:[a-z0-9]{8}"] } } },
      "gkos-gatekeeper-fs": { enabled: true, config: { roots: [] } },
    } },
