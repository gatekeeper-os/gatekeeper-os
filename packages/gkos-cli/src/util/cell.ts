/**
 * Cell identity: the paths, unit name, port, and environment for one GatekeeperOS cell (plan §3.3).
 *
...
export const CELLS_REGISTRY = join(homedir(), ".gkos", "cells.json");
 ...
 *
 * A named cell's port is chosen at `gkos cell create` time and must be stable across later invocations, so the
