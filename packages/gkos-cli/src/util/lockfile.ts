/**
 * `os/gkos.lock.json` — the pin, plus the reconciliation bookkeeping `gkos config apply` needs (plan §6.1).
 *
...
/** On-disk shape of `os/gkos.lock.json`. */
...
export function lockfilePath(cell: Cell): string {
  return join(cell.osDir, "gkos.lock.json");
}
