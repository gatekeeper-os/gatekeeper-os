...
function invoke(mode: string, method = 'os.status', badPort = false) {
  const root = mkdtempSync(join(tmpdir(), 'gkos-rpc-')); roots.push(root);
  ...
  writeFileSync(join(state, 'openclaw.json'), JSON.stringify({ gateway: { auth: { mode: 'token', token: { source: 'env', provider: 'default', id: 'GKOS_GATEWAY_TOKEN' } } } }));
  writeFileSync(join(state, '.env'), 'GKOS_GATEWAY_TOKEN=fixture-local-credential\n');
