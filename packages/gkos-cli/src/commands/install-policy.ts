...
import { evaluateInstall, type InstallMaterial, type InstallRules } from "@gatekeeper-os/shared";
...
export async function installPolicy(args: string[], globals: GlobalOptions): Promise<number> {
  ...
  try {
    ...
    for (const [key, expected] of [["OPENCLAW_STATE_DIR", cell.stateDir], ["OPENCLAW_CONFIG_PATH", cell.configPath], ["GKOS_CELL", cell.name]] as const) {
    ...
    const result = evaluateInstall(config.plugins?.entries?.["gkos-kernel"]?.config?.install, material);
