/**
 * `gkos install` — plan §10.3, as twelve idempotent steps.
 *
...
function templateRoot(): string {
  ...
  const fromSource = process.env.GKOS_FROM_SOURCE;
  if (fromSource && existsSync(join(fromSource, "config", "config.d"))) return join(fromSource, "config");
  throw new StepError("cannot locate config templates", "set GKOS_FROM_SOURCE to the repository checkout");
}
...
function pinnedVersion(): string {
  const candidates = [
    join(templateRoot(), "gkos.lock.json"),
    process.env.GKOS_FROM_SOURCE ? join(process.env.GKOS_FROM_SOURCE, "gkos.lock.json") : "",
  ].filter(Boolean);
  for (const path of candidates) {
    const lock = readJson<{ upstream?: { version?: string } }>(path);
    if (lock?.upstream?.version) return lock.upstream.version;
  }
  throw new StepError("cannot determine the upstream pin (gkos.lock.json not found)");
}
...
export function renderDropIn(cell: Cell, environmentFile?: string): string {
  const lines = [
    "# Written by `gkos install`. A drop-in never modifies upstream's unit file (INVARIANT 1).",
    "[Service]",
    "Environment=OPENCLAW_NO_AUTO_UPDATE=1",
    `Environment=GKOS_CELL=${cell.name}`,
  ];
  if (cell.name !== "default") {
    lines.push(`Environment=OPENCLAW_PROFILE=${cell.name}`, `Environment=OPENCLAW_GATEWAY_PORT=${cell.port}`);
  }
  // The Gateway token is referenced from config as ${GKOS_GATEWAY_TOKEN} and delivered by file, never by argv.
...
export async function install(args: string[], globals: GlobalOptions): Promise<number> {
  ...
  // [1/12] preflight — re-verified here even though installer/install.sh ran it, because `gkos install` is also
  ...
  if (!hasEnvVar(envPath, "GKOS_GATEWAY_TOKEN")) {
    appendEnvVar(envPath, "GKOS_GATEWAY_TOKEN", randomBytes(32).toString("hex"));
    keysChanged = true;
  }
  ...
  if (reconciled.conflicts?.length) {
    throw new StepError(
      ...
      "move those settings into os/config.d/90-local.json5, or run `gkos config apply --force`",
    );
  }
  ...
  if (process.platform === "darwin") {
    for (const [key, value] of [["OPENCLAW_NO_AUTO_UPDATE", "1"], ["GKOS_CELL", cell.name]]) {
  ...
  if (initialSchemaPin?.kernelSchema === 0) {
    if (existsSync(join(cell.osDir, "gkos.sqlite")) || existsSync(join(cell.osDir, "update-maintenance.json"))) throw new StepError("Legacy kernel schema requires explicit recovery");
  ...
  if (process.platform === "darwin") {
    ...
    if (writeFileIfChanged(join(cell.dropInDir, "gkos.conf"), renderDropIn(cell, environmentFile), 0o644)) serviceChanged = true;
