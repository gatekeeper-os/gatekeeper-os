...
export function mergeFragments(configD: string): Json {
  if (!existsSync(configD)) throw new StepError(`missing fragment directory ${configD}`, "run `gkos install` first");
 ...
 *
 * Separated from the command wrapper so `gkos install` can run step 8 and report whether it actually changed
...
export async function reconcile(
  ...
  // `gkos install` reconciles at step 8, before the service exists at step 9. Restarting a service that has
  ...
  try {
    ...
    if (lock) {
      ...
      // `gkos install` writes the lockfile in its last step; stash the digests so the guard is armed either way.
...
/** `gkos config apply` command wrapper: run the reconciliation and render its verdict. */
export async function configApply(args: string[], globals: GlobalOptions): Promise<number> {
  ...
  if (globals.json) {
    ...
    console.error("gkos config apply: OS-owned config paths changed outside the OS since the last apply:");
    ...
    console.log("gkos config apply: already in sync (no diff)");
  } else {
    for (const change of result.changes) console.log(`  ${change.kind.padEnd(7)} ${change.path}`);
    console.log(`gkos config apply: ${result.changes.length} path(s) changed${result.restarted ? "; gateway restarted" : ""}`);
  }
