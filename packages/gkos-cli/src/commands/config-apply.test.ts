...
describe("detectConflicts", () => {
  ...
  it("stores a digest, never the value, so the lockfile cannot leak an owned setting", () => {
    const digests = digestOwned(live({ "gateway.auth": { token: "${GKOS_GATEWAY_TOKEN}" } }));
    expect(JSON.stringify(digests)).not.toContain("GKOS_GATEWAY_TOKEN");
    expect(digests["gateway.auth"]).toBe(digest(canonicalize({ token: "${GKOS_GATEWAY_TOKEN}" })));
  });
});

describe("mergeFragments", () => {
  it("merges fragments in filename order, so 90-local wins over the baseline", () => {
    const dir = mkdtempSync(join(tmpdir(), "gkos-fragments-"));
  ...
  it("refuses an empty fragment directory rather than generating an empty config", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "gkos-fragments-empty-"));
    expect(() => mergeFragments(dir)).toThrow(/no config fragments/);
  });

  it("refuses a missing fragment directory", () => {
    expect(() => mergeFragments(join(tmpdir(), "gkos-does-not-exist-xyz"))).toThrow(/missing fragment directory/);
  });
});
