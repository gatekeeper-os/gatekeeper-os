/**
 * `gkos doctor` — host-layer health checks with fix hints (plan §9 Phase 1 step 5).
 *
...
export async function doctor(_args: string[], globals: GlobalOptions): Promise<number> {
  ...
  if (!existsSync(cell.osDir)) {
    findings.push({ id: "not-installed", severity: "error", message: `${cell.osDir} is missing`, hint: "gkos install" });
  }
  ...
  if (!installed) {
    findings.push({ id: "upstream-missing", severity: "error", message: "openclaw is not on PATH", hint: "gkos install" });
  ...
  const dropIn = process.platform === "darwin" ? join(homedir(), "Library", "LaunchAgents", `${cell.unit}.plist`) : join(cell.dropInDir, "gkos.conf");
  if (!existsSync(dropIn)) {
    findings.push({ id: "dropin-missing", severity: "error", message: `${dropIn} is missing`, hint: "gkos install" });
  }
  ...
  if (globals.json) {
    ...
    console.log(`gkos doctor: cell ${cell.name} is healthy`);
  } else {
