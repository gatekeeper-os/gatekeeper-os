...
export function blueprintEntry(b: Blueprint, workspace: string): Record<string,Json> {
  ...
  return {workspace,skills:b.skills,...(b.model?{model:{primary:b.model}}:{}),tools:{profile:b.toolPolicy.profile,
    ...(b.name==='researcher'?{alsoAllow:b.toolPolicy.allow}:{alsoAllow:[...new Set(['gkos-kernel',...b.toolPolicy.allow])]}),deny:b.toolPolicy.deny,
    ...(b.sandbox.mode!=='off'?{sandbox:{tools:{alsoAllow:[...new Set(['gkos-kernel',...b.toolPolicy.allow])]}}}:{}),
...
function templateRoot():string {
  ...
  if(process.env.GKOS_FROM_SOURCE)return resolve(process.env.GKOS_FROM_SOURCE);
...
function blueprintRoot(root:string,name:string):string {if(!ID.test(name))return fail('invalid blueprint name');return existsSync(join(root,'blueprints'))?join(root,'blueprints',name):join(root,'packages/gkos-blueprints',name);}
...
export async function applyBlueprint(cell:Cell,root:string,id:string,catalog:string[],applyConfig=()=>reconcile(cell.name)) {
  ...
  if(b.policy==='runtime'&&!runtimeCell(config)) {
    ...
    fail(`requires a runtime cell; run: gkos cell create ${name} --port ${port} --policy runtime`);
  }
...
/** `gkos blueprint list|lint|apply|diff`, no automatic grant, install, binding or approval. */
