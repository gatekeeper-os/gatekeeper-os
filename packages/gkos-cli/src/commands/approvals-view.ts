...
export function approvalView(value: unknown, preview = false): string {
  ...
  return ['ID       STATUS           KIND                           TITLE', ...rows, rows.length ? '' : 'No actions.', truncated ? 'More than 100 actions: response truncated; do not assume this is the full queue.' : '', preview ? '' : 'Use gkos approvals preview <IDs|all> to inspect actions before deciding.'].filter(Boolean).join('\n');
}
