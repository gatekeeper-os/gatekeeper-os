/**
 * `gkos backup create|restore` — cell backup and rollback on top of upstream's archive mechanism.
 *
 ...
 *    workspace tree to avoid self-inclusion. So archives go to `~/.gkos/backups/<cell>/`, not to
 ...
 *    and leaves activation to the operator. `gkos backup restore` performs exactly upstream's documented
...
export function backupDir(cell: Cell): string {
  return join(homedir(), ".gkos", "backups", cell.name);
}
...
export async function backup(args: string[], globals: GlobalOptions): Promise<number> {
  ...
  console.error("usage: gkos backup create | gkos backup restore <archive.tar.gz> --yes");
  return 2;
}
...
async function backupRestore(args: string[], globals: GlobalOptions): Promise<number> {
  ...
  if (!archive) throw new StepError("gkos backup restore requires an archive path");
  ...
  const staging = join(homedir(), ".gkos", "restore", `${cell.name}-${stamp}`);
  ...
  // are enforced here rather than left for `gkos doctor` to report afterwards.
