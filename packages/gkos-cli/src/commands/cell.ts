/**
 * `gkos cell create|list` — multi-cell support (plan §9 Phase 1 step 3, §10.5).
 *
...
export async function cell(args: string[], globals: GlobalOptions): Promise<number> {
  ...
  console.error("usage: gkos cell create <name> --port <n> [--policy messaging|runtime] [--yes] | gkos cell list");
  return 2;
}

async function cellCreate(args: string[], globals: GlobalOptions): Promise<number> {
  const name = args.find((a) => !a.startsWith("--"));
  if (!name) throw new StepError("gkos cell create requires a cell name");
  assertCellName(name);
  if (name === "default") throw new StepError("the default cell is created by `gkos install`");

  const policy = parseCellPolicy(optionValue(args, "--policy") ?? "messaging");
  const portArg = optionValue(args, "--port");
  if (!portArg) throw new StepError("gkos cell create requires --port <n>");
