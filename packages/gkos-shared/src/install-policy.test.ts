...
const material: InstallMaterial = {targetType:"plugin",sourcePath:"/staging/plugin",sourcePathKind:"directory",request:{kind:"plugin-install",mode:"install",requestedSpecifier:"npm:@gatekeeper-os/kernel@0.1.0"}};
describe("shared install boundary", () => {
  ...
  it("matches the entire source, escapes regex, and covers updates and skills", () => {
    const rules={allowSources:["npm:@gatekeeper-os/*"]};
    expect(evaluateInstall(rules,material).decision).toBe("allow");
    for (const requestedSpecifier of ["npm:@gatekeeper-os-evil/kernel","https://evil/npm:@gatekeeper-os/kernel", "npm:@gatekeeper-os/kernel\n"]) expect(evaluateInstall(rules,{...material,request:{...material.request,requestedSpecifier}}).decision).toBe("block");
  ...
  it("does not let registry namespace wildcards authorize URL, file or alias overrides", () => {
    for (const requestedSpecifier of ["npm:@gatekeeper-os/kernel@https://evil.invalid/p.tgz", "npm:@gatekeeper-os/kernel@/tmp/evil", "npm:@gatekeeper-os/kernel@npm:evil@1", "clawhub:@gatekeeper-os/kernel@file:/tmp/evil", "@gatekeeper-os/kernel@../evil"]) {
      expect(evaluateInstall({allowSources:["npm:@gatekeeper-os/*","clawhub:@gatekeeper-os/*","@gatekeeper-os/*"]},{...material,request:{...material.request,requestedSpecifier}}).decision).toBe("block");
    }
  });
