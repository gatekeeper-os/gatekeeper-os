...
function fixture(){
  ...
  for(const id of ["clawos-kernel","gatekeeper-fs"]){mkdirSync(join(source,id));writeFileSync(join(source,id,"package.json"),JSON.stringify({version:"0.1.0"}));}
  writeFileSync(join(source,"install-policy.mjs"), "// standalone policy fixture\n");
  writeFileSync(join(source,"catalog.json"),JSON.stringify({version:1,gatekeepers:[{pluginId:"gatekeeper-fs"}]}));
...
it("projects inert defaults, retains local overrides and repeats without changes",()=>{
  ...
  expect(cfg.plugins.entries["gatekeeper-fs"].config.roots).toEqual([]);
  expect(cfg.plugins.entries["clawos-kernel"].config.install.allowSources).toEqual([]);
