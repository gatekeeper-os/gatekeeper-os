...
describe("getPath", () => {
  it("reads a nested path and returns undefined for a missing one", () => {
    const config = { plugins: { entries: { "clawos-kernel": { enabled: true } } } };
    expect(getPath(config, "plugins.entries.clawos-kernel")).toEqual({ enabled: true });
    expect(getPath(config, "plugins.entries.gatekeeper-fs")).toBeUndefined();
    expect(getPath(config, "plugins.entries.clawos-kernel.enabled.deeper")).toBeUndefined();
  });
});
