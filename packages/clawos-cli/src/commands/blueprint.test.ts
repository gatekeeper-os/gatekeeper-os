...
describe('blueprint safety and provisioning',()=>{
  it('refuses coder in messaging cell before files or upstream writes, even if fragments claim runtime',async()=>{
    ...
    await expect(applyBlueprint(cell,join(templates,'coder'),'dev',catalog,applyConfig)).rejects.toThrow('clawos cell create blueprint-test-runtime --port 18790 --policy runtime');
