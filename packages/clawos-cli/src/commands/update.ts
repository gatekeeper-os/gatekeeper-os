...
function journalPath(cell: Cell): string { return join(homedir(), '.clawos', 'updates', cell.name, 'current.json'); }
function dropIn(cell: Cell): string { return join(cell.dropInDir, 'zz-clawos-runtime.conf'); }
...
function snapshot(cell: Cell, j: UpdateJournal): void {
    const output = join(homedir(), '.clawos', 'updates', cell.name, j.id, 'backup-' + randomBytes(4).toString('hex'));
    ensureDir(output);
...
export function updateEffects(cell: Cell, lock: Lockfile, j: UpdateJournal, conformanceScript: string): UpdateEffects {
    ...
    return {
        ...
        async conformance() {
            ...
            const state = join(homedir(), '.clawos', 'updates', cell.name, j.id, 'conformance-state');
            ensureDir(state);
            const config = join(state, 'openclaw.json');
            writeFileSync(config, '{}\n', { mode: 0o600 });
            const verdict = join(state, 'verdict.json');
            command(process.execPath, [conformanceScript], { ...envFor(cell), OPENCLAW_STATE_DIR: state, OPENCLAW_CONFIG_PATH: config, OPENCLAW_PROFILE: 'clawos-update-conformance', CLAWOS_UPDATE_BINARY: j.candidateBinary, CLAWOS_UPDATE_RUN_ID: j.id, CLAWOS_UPDATE_TARGET: j.target, CLAWOS_UPDATE_VERDICT: verdict }, 15 * 60000);
...
async function restoreRuntime(cell: Cell, j: UpdateJournal): Promise<void> {
    ...
    const scratch = join(homedir(), '.clawos', 'updates', cell.name, j.id, 'restore-' + randomBytes(4).toString('hex'));
    ensureDir(scratch);
    ...
    const previousLock = JSON.parse(readFileSync(join(state, 'os/clawos.lock.json'), 'utf8')) as Lockfile;
...
async function updateCommand(args: string[], globals: GlobalOptions): Promise<number> {
    ...
    if (existsSync(path) && !['committed', 'rolled-back', 'blocked'].includes(readUpdate(path).state))
        throw new StepError('Unfinished update; run clawos rollback --yes');
    const id = randomBytes(16).toString('hex');
    const journal: UpdateJournal = { format: 1, id, cell: cell.name, from: lock.upstream.version, target, kernelSchema: lock.kernelSchema, state: 'preparing', step: 1, completed: [], startedAt: new Date().toISOString(), previousBinary: currentBinary(cell), candidateBinary: join(homedir(), '.clawos', 'runtimes', cell.name, id, 'bin/openclaw'), previousDropIn: existsSync(dropIn(cell)) ? readFileSync(dropIn(cell), 'utf8') : null };
...
export async function rollback(args: string[], globals: GlobalOptions): Promise<number> {
    if (args.length || !globals.yes)
        throw new StepError('usage: clawos rollback --yes [--cell name]');
