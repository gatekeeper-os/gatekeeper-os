...
- `gkos gatekeeper connect <vendor>`: authenticated, cell-bound one-use browser start URL. The kernel owns the two-stage kit nonce; vendor adapters own provider PKCE/account checks and encrypted tokens. Static filesystem accounts require no external login.
- `gkos approvals list`: bounded `{actions, requests}` response, including action descriptions/previews and agent resource requests.
- `gkos approvals apply|reject|revert <IDs|all>`: serialized decisions through the originating grant/session, with durable uncertain-outcome records; applied actions alone may be reverted.
- `gkos approvals grant|reject-request <IDs|all>`: decide stored agent access requests; agents cannot mint grants.
