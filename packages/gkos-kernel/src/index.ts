// GatekeeperOS kernel plugin entry (plan §5.1). Every SDK shape used here type-checks against openclaw@2026.9.2.
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    if (["cli-metadata", "discovery", "full"].includes(api.registrationMode)) {
      api.registerCli(({ program }) => mountOperatorCli(program), { descriptors: [{ name: "os", description: "GatekeeperOS kernel administration", hasSubcommands: true, machineOutput: ({ argv }) => argv.includes("--json") }] });
    }
