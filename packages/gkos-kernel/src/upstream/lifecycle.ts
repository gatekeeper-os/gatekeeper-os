...
export function finishOperatorCommand(event:HookEvent<"reply_dispatch">,ctx:HookCtx<"reply_dispatch">,text:string):NonNullable<Awaited<HookResult<"reply_dispatch">>>{
  const queuedFinal=text.length>0&&event.sendPolicy!=="deny"&&!event.suppressUserDelivery?ctx.dispatcher.sendFinalReply({text:text.slice(0,8192)}):false;
  ctx.recordProcessed("completed",{reason:"gkos operator command"});ctx.markIdle("gkos operator command");
