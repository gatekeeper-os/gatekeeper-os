...
export function sendOperatorDigest(target:{channel:string;target:string},actions:number,requests:number):Promise<void>{
  ...
  const message=`GatekeeperOS: ${actions} pending action(s), ${requests} access request(s). Use /approvals in your private operator conversation or gkos approvals list.`;
