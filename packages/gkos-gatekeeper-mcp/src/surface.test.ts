...
it.each(["../demo", "demo/", "demo?token=x", "demo#x", "demo%2fextra", "Demo", "", "demo/../demo", "demo%00", "demo:443", "demo\n"])("rejects ambiguous resource suffix %j", suffix => {
  expect(() => parseProposedResourceUrl("https://mcp.gatekeeper-os.invalid/servers/" + suffix, ["demo"])).toThrow();
});
it("rejects alternate origin/userinfo and unconfigured resources", () => {
  for (const url of ["http://mcp.gatekeeper-os.invalid/servers/demo", "https://mcp.gatekeeper-os.invalid:443/servers/demo", "https://user@mcp.gatekeeper-os.invalid/servers/demo", "https://evil.invalid/servers/demo"]) expect(() => parseProposedResourceUrl(url, ["demo"])).toThrow();
...
it("registers the reviewed observation only while native append remains gated", () => {
  const entry=readFileSync(new URL("./index.ts", import.meta.url), "utf8");
  const manifest=JSON.parse(readFileSync(new URL("../openclaw.plugin.json", import.meta.url), "utf8"));
  expect(entry).toContain("defineGatekeeper");expect(entry).not.toContain("registerTool");
  expect(manifest.contracts.tools).toEqual(["gk_mcp_demo_read_note"]);expect(manifest.activation.onStartup).toBe(true);
});
