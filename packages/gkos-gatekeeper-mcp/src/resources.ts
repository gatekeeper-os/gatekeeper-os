...
export const proposedResourcePattern = "https://mcp.gatekeeper-os.invalid/servers/:server";
...
export function proposedResourceUrl(server: string): string {
  ...
  return `https://mcp.gatekeeper-os.invalid/servers/${server}`;
}
...
export function parseProposedResourceUrl(raw: string, knownServers: readonly string[]): string {
  const prefix = "https://mcp.gatekeeper-os.invalid/servers/";
