...
import { TokenStore } from "@gatekeeper-os/gatekeeper-kit";
...
const dirs: string[] = [];
const binding = {id: "demo" as const, endpoint: "https://example.com/mcp", operatorId: "operator", credentialEnv: "GKOS_MCP_FIXTURE"};
const logger = {debug:vi.fn(),info:vi.fn(),warn:vi.fn(),error:vi.fn()};
function setup(inspect = vi.fn(async (_endpoint:string,_bearer:string) => ({ tools: structuredClone(reviewedInventory) }))) {
  const root = mkdtempSync(join(tmpdir(), "gkos-mcp-boundary-")); dirs.push(root);
  mkdirSync(join(root,"os")); writeFileSync(join(root,"os","cell.key"), Buffer.alloc(32,7).toString("base64")+"\n", {mode:0o600});
  process.env.GKOS_MCP_FIXTURE = "synthetic-fixture-credential-not-real";
...
afterEach(()=>{ for(const path of dirs.splice(0)) rmSync(path,{recursive:true,force:true}); delete process.env.GKOS_MCP_FIXTURE; vi.clearAllMocks(); });
...
it("requires explicit credentials; stores encrypted exact binding; restores without environment",async()=>{
  const {root,vendor,inspect,config}=setup(); delete process.env.GKOS_MCP_FIXTURE;
  ...
  expect(readFileSync(join(root,"os","gatekeepers","mcp","accounts",files[0]!),"utf8")).not.toContain(process.env.GKOS_MCP_FIXTURE);
  delete process.env.GKOS_MCP_FIXTURE;
...
it("prevents binding rotation from reusing persisted credentials",async()=>{
  const {root,vendor,inspect}=setup();await vendor.createAccount("operator");delete process.env.GKOS_MCP_FIXTURE;
