/** Upstream-mounted commands use the installed clawos operator client, never an unstarted local kernel store. */
...
export function operatorCell(env: NodeJS.ProcessEnv, home = homedir()): string {
  const cell = env.CLAWOS_CELL ?? env.OPENCLAW_PROFILE ?? "default";
...
export function runOperatorCli(cell: string, args: string[]): void {
  const result = spawnSync('clawos', [...args, '--cell', cell, '--json'], {
