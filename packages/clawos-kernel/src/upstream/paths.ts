...
export function osPaths() {
  ...
  return { stateDir: stateDir(), os, sqlite: join(os, "clawos.sqlite"), auditDir: join(os, "audit"), configD: join(os, "config.d"),
    lock: join(os, "clawos.lock.json"), gatekeepers: join(os, "gatekeepers"), cellKey: join(os, "cell.key"), logs: join(os, "logs") };
}
