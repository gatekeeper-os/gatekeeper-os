...
export class Store {
  ...
  /** Synchronous actions start uncertain atomically, before any provider effect can occur. */
  addAction(instance:string,actionId:number,d:ActionDescription,initialStatus:"pending"|"failed"="pending"):PendingAction{ this.db.prepare("INSERT OR IGNORE INTO actions(gatekeeperInstance,actionId,descriptionJson,status,submittedAt,error) VALUES(?,?,?,?,?,?)").run(instance,actionId,JSON.stringify(d),initialStatus,Date.now(),initialStatus==="failed"?"Native action outcome unconfirmed; reconcile before retry":null); return this.db.prepare("SELECT * FROM actions WHERE gatekeeperInstance=? AND actionId=?").get(instance,actionId) as unknown as PendingAction; }
